...
public class ItemServiceDbImpl implements ItemService {
    ...
    public ItemDto create(ItemDto itemDto, long userId) {
        if (itemDto.getAvailable() == null || itemDto.getDescription() == null || itemDto.getName() == null) {
            throw new EmptyFieldException("Пустые поля в ItemDto!");
        }
        if (itemDto.getName().isEmpty() || itemDto.getDescription().isEmpty()) {
            throw new EmptyFieldException("Пустые поля в ItemDto!");
        }
        UserDto userFromDb = checkingUserId(userId);
        log.debug("Создание вещи : {}; для пользователя {}", itemDto, userId);
    ...
    public ItemDto update(ItemDto itemDto, long userId) {
        ...
        log.debug("обновление вещи : {}; для пользователя {}", itemDto, userId);
    ...
    public ItemDto getItemById(long itemId, long userId) {
        ...
        if (!bookingsForItem.isEmpty() && !commentsForItem.isEmpty()) {
            return toItemDtoWithBookingsAndComments(itemRepository.findById(itemId)
                    .orElseThrow(() -> new EntityNotFoundException("Не вещи с id: " + itemId)),
    ...
    public Collection<ItemDto> getItemsByUserId(long userId) {
        ...
        List<CommentDto> commentsToUserItems = commentRepository.findAllByItemsUserId(userId,
        ...
        log.debug("Получение вещей по id пользователя : {} ", userFromDb.getId());
    ...
    public CommentDto addCommentToItem(Long userId, Long itemId, CommentDto commentDto) {
        ...
        List<BookingDto> bookings = bookingRepository.findAllByUserIdAndItemIdAndEndDateIsPassed(userId,
                        itemId, LocalDateTime.now())
                .stream()
                .map(BookingMapper::toBookingDto)
                .collect(Collectors.toList());
        if (bookings.isEmpty()) {
            throw new IncorrectDataException("У этого пользователя нет бронирования");
        }
        ItemDto item = getItemById(itemId, userId);
        commentDto = toCommentDto(commentRepository.save(CommentMapper.toCommentDb(commentDto, toUser(author),
                toItem(item))));
        return commentDto;
    }
